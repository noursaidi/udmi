...
cat $site_path/devices/AHU-1/metadata.json | jq -r '.discovery|={"families": {"bacnet":{}}}' | sponge $site_path/devices/AHU-1/metadata.json 

# Wait for Clearblade ..
sleep 10

echo Enable the gateway to auto-provision
bin/mapper AHU-1 provision

echo Kick off a discovery run for the gateway
bin/mapper AHU-1 discover
